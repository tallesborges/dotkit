...
- `the DotNS Registry … implements neither known setSubnodeOwner tuple` on a subnode or executable write → the chain moved past DotNS v0.7's `(bytes32,string,string,address,bool)` record. dotkit probes both known shapes, so this is a real ABI change needing a dotkit update, not a retry or a funding problem.
